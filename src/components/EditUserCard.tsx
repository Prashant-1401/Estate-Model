...
import { X, User, Mail, Phone, Shield, Key, Save, UserCheck, UserX } from "lucide-react";
...
export function EditUserCard({ isOpen, onClose, onSubmit, user }: EditUserCardProps) {
      ...
      >
        ...
        >
          <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-[#E2E8F0] shrink-0">
            <div>
              <h2 className="text-lg sm:text-xl font-semibold text-[#0F172A]">Edit User</h2>
              <p className="text-xs sm:text-sm text-[#64748B] mt-0.5">Update team member details</p>
            </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  {formData.status === "Active"
                    ? <UserCheck size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#22C55E] pointer-events-none" />
                    : <UserX size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#EF4444] pointer-events-none" />}
